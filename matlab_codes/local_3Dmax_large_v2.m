function [center, dx, dy, dz] = local_3Dmax_large_v2(u1,xyr,zr)
% Difference with local_3Dmax_large: threshol of xy & z are parameters not fixed
...
while I>0
    ...
    inx3 = label_neibs(u1, inx3, xc, yc, zc, a, b, c,xyr,zr);
    inx3 = inx3>0;
    photons = sum(u1(inx3));
...
function inx3 = label_neibs(h2d, inx3, xc, yc, zc, a, b, c,xyr,zr)
...
[dx, dy, dz] = ndgrid(xc-xyr:xc+xyr,yc-xyr:yc+xyr, zc-zr:zc+zr);
pp = (xyr*2+1)^2*zr + ceil((xyr*2+1)^2/2);
dx(pp) = []; dy(pp) = []; dz(pp) = []; % the position of inx3
...
for k = 1:numel(dy)
    if(inx3(dx(k),dy(k),dz(k))==0 && h2d(dx(k),dy(k),dz(k))>0 && h2d(dx(k),dy(k),dz(k))<=h2d(xc,yc,zc))
        ...
        inx3 = label_neibs(h2d, inx3, dx(k),dy(k),dz(k),a,b,c,xyr,zr);
    end
end
end
