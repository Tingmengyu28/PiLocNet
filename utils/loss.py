...
class CEL0Loss(nn.Module):
    def __init__(self,opt):
        ...
        norm_ai_path = f'./matlab_codes/norm_ai_D41_L7.mat'
...
#     PSF_path = './utils/rPSF_A_L5_41slices.mat'
#     # PSF_path = './utils/rPSF_A_0.05I_L5_21slices.mat'
...
#         pred = fft_conv(normgrid, self.PSF, padding=(pd_d,pd_h,pd_w),padding_mode='reflect')[:,0,125,:]
...
def PSF_matrix():
    ...
    # 2023-02-22 modify to next 
    PSF_path = './utils/rPSF_A_L7_41slices.mat'
    PSF_mat = scipy.io.loadmat(PSF_path)
    PSF = torch.from_numpy(PSF_mat['A']).permute(2,0,1).to('cuda') # from [H,W,D] -> [D,H,W]
...
class ForwardLoss(nn.Module):
    # 2021-08-17 Forward loss as fidelity term, ||A*y^-I0||^2
    # 2021-08-31 Implement conv3d using fft, details found in fft_conv.py
    # 2023-02-22 Implement conv(A,X) using torch.fft.fftn with torch 1.10

    def __init__(self,opt):
        super(ForwardLoss,self).__init__()
        self.PSF = PSF_matrix() # also the new version

    def forward(self,input,target3d=None,target2d=None):
        '''
        input = 3d predict grid [N,D,H,W], D is # of channel, considered as depth here
        target = 2d observed image
        pred = 2d image generated from input and PSF
        ...
        # V1: if add perturbation in A => conv(A+0.05I,X)
        # self.PSF = self.PSF + 0.05*?

        fA = torch.fft.fftn(self.PSF)
        fX = torch.fft.fftn(input,dim=tuple(range(1,input.ndim)))
        fAX = fA*fX
        AX_beforeshift = torch.fft.ifftn(fAX,dim=tuple(range(1,fAX.ndim)))
        AX = torch.fft.ifftshift(torch.fft.fftshift(AX_beforeshift),(0,1)).float()

        # V2: if add perturbation like conv(A,X) + 0.05X
        # AX = AX + 11*input

        TAX = AX[:,-1,:,:] # the T operator extract the last slice as observed image

        mse_fft = nn.MSELoss()(TAX,target2d)
...
class ForwardKL(nn.Module):
    ...
    def forward(self,input,target3d=None,target2d=None):
        ...
        kdl = TAX - target2d * torch.log(TAX + self.opt.background)
        kdl_loss = torch.mean(kdl)
...
class calculate_loss(nn.Module):
    ...
    def __init__(self, opt, loss_type, weight):
        ...
        self.operators = {'mse3d':MSE3D, 'cel0':CEL0Loss, 'klnc':klncLoss, 'forward':ForwardLoss, 'forwardKL':ForwardKL }
        self.ops = {}
        self.weights = weight
